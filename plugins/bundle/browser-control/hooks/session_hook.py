...
def _is_browser_mission_message(item: Any) -> bool:
    ...
    return isinstance(metadata, dict) and metadata.get("browser_mission") is True


def _is_browser_mission_complete_message(item: Any) -> bool:
    if not _is_browser_mission_message(item):
        return False
    metadata = getattr(item, "metadata", None)
    status = str(metadata.get("browser_mission_status") or "").strip().lower()
    return status == "complete"


def _request_context(ctx: Any) -> dict[str, Any] | None:
...
def _mark_browser_mission_completed(ctx: Any) -> None:
    ...
    if request_context is not None:
        request_context["browser_control_mission_completed"] = True


def _browser_mission_completed(ctx: Any) -> bool:
    extras = getattr(ctx, "extras", {}) or {}
    if extras.get("browser_control_mission_completed") is True:
        return True
    request = getattr(ctx, "request", None)
    request_context = getattr(request, "request_context", None)
    return (
        isinstance(request_context, dict)
        and request_context.get("browser_control_mission_completed") is True
    )


def _browser_mission_message_text(item: Msg) -> str:
    return "\n".join(
        _block_text(block) for block in _message_content(item) if _block_text(block)
    )
...
def _browser_mission_task(ctx: HookContext) -> str:
    extras = getattr(ctx, "extras", {}) or {}
    value = str(extras.get("browser_control_mission_task") or "")
    if value:
        return value
    request = getattr(ctx, "request", None)
    request_context = getattr(request, "request_context", None)
    if isinstance(request_context, dict):
        return str(request_context.get("browser_control_mission_task") or "")
    return ""
    ...
    async def run(self, ctx: HookContext) -> HookResult:
        ...
        async def mission_reply_stream(*, inputs=None, **_kwargs):
            async for item in run_browser_mission(
                ...
                task=_browser_mission_task(ctx),
            ):
                if _is_browser_mission_message(item):
                    if _is_browser_mission_complete_message(item):
                        _mark_browser_mission_completed(ctx)
...
class BrowserControlFinalizeHook(LifecycleHook):
    ...
    async def run(self, ctx: HookContext) -> HookResult:
        ...
        try:
            ...
            if ctx.error is not None or _browser_mission_completed(ctx):
