...
PROMPT_PATH = Path(__file__).resolve().parents[1] / "prompts" / "control_guidance.md"
...
def _remember_browser_mission_prd(
    *,
    extras: dict[str, Any],
    ...
    extras["browser_control_mission_task"] = task

    if request_context is not None:
        request_context["browser_control_mission_prd_path"] = str(prd_path)
        request_context["browser_control_mission_task"] = task
...
def initialize_browser_mission_prd(ctx: Any, user_task: str) -> Path:
    ...
    if request is not None:
        raw_context = getattr(request, "request_context", None)
        if not isinstance(raw_context, dict):
            raw_context = {}
            setattr(request, "request_context", raw_context)
        request_context = raw_context

    existing_path = str(
        extras.get("browser_control_mission_prd_path")
        or (
            request_context.get("browser_control_mission_prd_path")
            if request_context is not None
            else ""
        )
        or "",
    ...
    if existing_path:
        prd_path = Path(existing_path).expanduser().resolve()
        if prd_path.exists():
            task = str(
                extras.get("browser_control_mission_task")
                or (
                    request_context.get("browser_control_mission_task")
                    if request_context is not None
                    else ""
                )
                or user_task
