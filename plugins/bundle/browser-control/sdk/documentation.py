...
Run SDK snippets through ``browser(code=...)``. Connect to the user's Chrome
session explicitly when the task requires login state or existing tabs:

    browser = await Browser.connect(context="user")

For public-web research, use the default arbitration path:

    browser = await Browser.connect(context="auto")

If the Chrome Extension bridge is disconnected for a user-context task, stop
and report the bridge blocker. Do not reroute logged-in, cart, account, or
existing-tab work to an isolated browser.
...
whether the current run created it. In cart/list cleanup workflows, if a
...
LLM-safe call patterns. Use these as separate ``browser(code=...)`` calls.
Do not paste the full sequence into one call. Each call should contain at
most one state-changing browser action:

    # Call 1: open and observe
    tab = await browser.tabs.open("https://example.com")
    snap = await tab.snapshot()
    print(snap.text)

    # Call 2: one action, optional wait, then observe
    await tab.click(ref="r1_e22")
    await tab.wait_for(2)
    snap = await tab.snapshot()
    print(snap.text)

    # Call 3: another action only after the previous observation is returned
...
save-and-confirm, use one ``browser(code=...)`` turn per mutating action. If
ObservationRequired appears, do not retry the same multi-action call; observe
once, then issue exactly one next state-changing action in a new call.
...
Keep each browser call narrow: perform at most one mutating browser action,
optionally wait for it to settle, then take a fresh snapshot. Do not batch
click/type/click sequences in one ``browser(code=...)`` call. If a click, type,
...
may include an href used by the SDK for reliable same-tab navigation. Quoted
