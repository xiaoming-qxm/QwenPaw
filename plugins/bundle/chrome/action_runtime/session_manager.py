# -*- coding: utf-8 -*-
"""Chrome session lifecycle helpers."""
    ...
    for session in sessions.values():
        for attribute, registered in (
            ...
            try:
                ...
                logger.debug(
                    "Failed to remove chrome prompt handler",
                    exc_info=True,
                )
...
async def _control_prepare_session_events(
    ...
    *,
    ...
    try:
        ...
        logger.debug(
            "Failed to enable Page events for chrome tab %s",
            tab_id,
            exc_info=True,
        )
...
async def _control_get_existing_session(
    ...
    *,
    ...
    try:
        ...
        logger.debug(
            "Discarded stale chrome session tab=%s holder=%s",
            tab_id,
            holder_id,
            exc_info=True,
        )
