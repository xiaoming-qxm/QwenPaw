(() => {
  const CONTENT_LOADED_FLAG = "__qwenpawChromeContentLoaded";
  if (window[CONTENT_LOADED_FLAG]) {
    return;
  }
  window[CONTENT_LOADED_FLAG] = true;

  const SOURCE = "qwenpaw-chrome-content";
  const HOST_ID = "qwenpaw-chrome-host";
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (!message || message.source !== "qwenpaw-chrome") {
      return false;
    }
