...
class ChromeLifecycleCleanupHook(LifecycleHook):
    ...
    async def run(self, ctx: HookContext) -> HookResult:
        ...
        try:
            result = await cleanup_chrome_request_resources(
                session_id=session_id,
                root_session_id=root_session_id,
                request_scope_key=request_scope_key,
                workspace_id=_workspace_id(ctx),
            ...
            logger.debug(
                "chrome_lifecycle_cleanup: failed",
                exc_info=True,
            )
        else:
            cleanup_errors = int((result or {}).get("cleanup_errors", 0))
...
def _cleanup_reason(ctx: HookContext) -> str:
    ...
    reason = "finally"
    if extras.get("chrome_shutdown"):
        reason = "shutdown"
    elif extras.get("chrome_disconnected"):
        reason = "bridge_disconnect"
    elif extras.get("chrome_blocked"):
        reason = "blocked"
...
def _record_cleanup_trace(
    *,
    ...
    record_browser_trace_event(
        ...
        action="chrome_lifecycle_cleanup",
        status=status,
...
def _record_cleanup_start_trace(
    *,
    ...
    record_browser_trace_event(
        ...
        requested_context="user",
        selected_context="user",
        action="chrome_lifecycle_cleanup_start",
        status="started",
        metadata={
            "cleanup_reason": cleanup_reason,
