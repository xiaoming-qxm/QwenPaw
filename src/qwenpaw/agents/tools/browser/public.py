...
async def browser_use(  # pylint: disable=R0911,R0912
    ...
    Args:
        ...
        allow_new_context (bool):
            Control-mode click policy. Default False keeps target=_blank and
            window.open results inside the controlled tab to avoid stealing
            focus. Set True only when the user's task explicitly needs the
            clicked link or form to create a separate tab/window.
        port (int):
