...
try:
    ...
    AsyncPlaywrightError = RuntimeError
    SyncPlaywrightError = RuntimeError

_PLAYWRIGHT_LIFECYCLE_ERRORS = (
    OSError,
    ImportError,
    RuntimeError,
...
_PROCESS_CLEANUP_ERRORS = (OSError, RuntimeError, subprocess.TimeoutExpired)
...
async def _configure_download_behavior(state: dict) -> None:
    ...
    try:
        ...
        # CDP download configuration can fail when the browser is closing.
        logger.debug(
        ...
        if cdp is not None:
            try:
                ...
                # CDP session detach is best-effort during browser teardown.
                logger.debug(
...
if _USE_SYNC_PLAYWRIGHT:
    ...
    def _get_executor() -> futures.ThreadPoolExecutor:
        """Return sync executor; only available in sync Playwright mode."""
        raise RuntimeError("sync Playwright executor is not available")
    ...
    try:
        ...
        for ws_state in list(_workspace_states.values()):
            if _is_browser_running(ws_state):
                ...
                except _PLAYWRIGHT_LIFECYCLE_ERRORS:
                    # Playwright shutdown may race with interpreter teardown.
                    pass
    except _PLAYWRIGHT_LIFECYCLE_ERRORS:
        # Event-loop or Playwright cleanup can fail during process exit.
        pass
...
async def _stop_playwright_instance(pw: Any) -> None:
    ...
    try:
        ...
        # Playwright stop is best-effort for partially-started drivers.
        pass
...
def _sync_browser_close(state: dict):
    ...
    if state["_sync_browser"] is not None:
        try:
            ...
            # Sync browser close can race with native process shutdown.
            pass
        ...
        try:
            ...
            # Persistent context close can race with native process shutdown.
            pass
    if state["_sync_playwright"] is not None:
        try:
            state["_sync_playwright"].stop()
        except _PLAYWRIGHT_LIFECYCLE_ERRORS:
            # Sync Playwright driver stop is best-effort during teardown.
            pass
...
async def _wait_for_cdp_ready(
    ...
    while time.monotonic() < deadline:
        try:
            ...
            # CDP readiness probe may hit connection errors or invalid JSON.
            last_error = exc
...
async def _start_managed_cdp_browser(
    ...
    try:
        ...
        # Managed CDP startup can fail while launching or attaching Playwright.
        await _stop_playwright_instance(pw)
        try:
            if proc.poll() is None:
                proc.kill()
                await asyncio.to_thread(proc.wait, 5)
        except _PROCESS_CLEANUP_ERRORS:
            # Process cleanup is best-effort after a failed launch.
            pass
        raise
...
async def _stop_owned_browser_process(state: dict) -> bool:
    ...
    try:
        ...
        try:
            ...
            # Process kill/wait can fail if Chrome exits concurrently.
            return False
    except (OSError, RuntimeError):
        # Process termination can fail if the process already exited.
        return False
...
async def _get_tab_info_list(state: dict) -> list[dict[str, str]]:
    ...
    for pid, p in list(pages.items()):
        ...
        except _PAGE_CONTEXT_ERRORS:
            # Page access may fail if the tab detached or the browser crashed.
        ...
        if state["_sync_context"] is not None:
            ...
            if browser:
                try:
                    ...
                    # Browser connection probing can fail after native teardown.
                    is_connected = False
    ...
    try:
        if _USE_SYNC_PLAYWRIGHT:
            ...
            try:
                ...
                # Managed CDP launch failure falls back to regular Playwright start.
                await _action_start(
                    state,
        ...
        return True
    except Exception as e:  # TODO: split backend startup hook failures by source.
...
__all__ = [
    ...
    "_HEADLESS_VERIFICATION_WARNING",
    "_MAX_DIRECT_URL_DOWNLOAD_BYTES",
