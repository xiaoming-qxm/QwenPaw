...
_CONTROL_POINT_SNAP_EXCEPTIONS = RECOVERABLE_CONTROL_EXCEPTIONS + (CDPRelayError,)
...
def _control_quad_area(quad: Any) -> float:
    if not isinstance(quad, list) or len(quad) < 8:
        return 0.0
    points = [(float(quad[index]), float(quad[index + 1])) for index in range(0, 8, 2)]
    area = 0.0
...
async def _control_selector_target(
    ...
    if not isinstance(matched_node_id, int) or matched_node_id <= 0:
        raise TargetResolutionFailed(f"No element matches selector: {selector}")
