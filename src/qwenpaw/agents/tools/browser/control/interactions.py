...
_network_quiescence_wait_impl = _default_network_wait
...
def set_network_quiescence_wait(func: Any) -> None:
    ...
    async def monitor_factory(
        ...
        return _DeferredNetworkWaitMonitor(func, session, bridge, state, tab_id)
    ...
    *,
    ...
    if navigation_occurred:
        ...
        instruction = (
            ...
            "declare success or failure from an unchanged local badge, counter, "
            "or control. Verify by waiting and observing, reloading and "
            "observing, or reading an authoritative state view."
        )
    else:
        message = (
    ...
    if isinstance(network_metadata, dict) and async_requests > 0:
        ...
        if not navigation_occurred:
            payload["state_verification"] = _control_state_verification_payload(
                status="pending",
...
async def click_control(
    ...
    *,
    ...
    tab_id = _control_tab_id(
        _control_page_id(state, str(kwargs.get("page_id", ""))), kwargs.get("index", -1)
    )
    ...
    if (
        ...
        x, y = await _control_snap_to_element(
            session, float(x_param), float(y_param), width, height
        )
    else:
    ...
    try:
        waiter = _control_create_action_transition_waiter(
            bridge, before_tabs=before_tabs, source_tab_id=tab_id
        )
        ...
        if transition is not None:
            ...
            _control_mark_observation_required(
                state, transition_tab_id or tab_id, action="click"
            )
        ...
        after_tabs = await _control_discover_tabs_safe(bridge) if before_url else None
...
async def type_control(
    ...
    *,
    ...
    if not target and selector:
        target = await _control_selector_target(session, selector)
    if target:
        x, y = await _control_resolve_point(session, target, ref=ref or selector)
        await _control_click_at(session, x, y, "Focus")
    text_to_type = str(kwargs.get("text", ""))
    await _control_show_keyboard(session, text=text_to_type, status_text="Type")
    ...
    return await _finalize_keyboard_action(
        ...
        request_context,
        "type",
        "Text was entered. Observe the page before deciding whether to press Enter, click a submit button, or take another action.",
    )
...
async def press_key_control(
    ...
    *,
    ...
    return await _finalize_keyboard_action(
        ...
        request_context,
        "press_key",
        "The key press completed. Observe the page before taking another action.",
    )
...
async def _finalize_keyboard_action(
    ...
    if transition is not None:
        transition_tab_id = _control_int_tab_id(transition.get("tab_id"))
        _control_mark_observation_required(
            state, transition_tab_id or tab_id, action=action
        )
        return _json_response(transition)
