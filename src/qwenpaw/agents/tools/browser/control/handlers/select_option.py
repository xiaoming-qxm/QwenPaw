...
_SELECT_FUNCTION = 'function(value) { this.value = String(value); this.dispatchEvent(new Event("input", { bubbles: true })); this.dispatchEvent(new Event("change", { bubbles: true })); return this.value; }'
...
class SelectOptionHandler:
    ...
    async def execute(
        self, state: ControlState, *, holder_id: str, bridge: Any, **kwargs: Any
    ):
        try:
            ...
            node_params = await _select_node_params(state, session, tab_id, kwargs)
            resolved = await session.send("DOM.resolveNode", node_params)
            remote_object = resolved.get("object") if isinstance(resolved, dict) else {}
            object_id = (
                remote_object.get("objectId") if isinstance(remote_object, dict) else ""
            )
            if not object_id:
                raise TargetResolutionFailed("Unable to resolve select element")
            await session.send(
            ...
            return _json_response({"ok": False, "mode": "control", "error": str(exc)})
...
async def _select_node_params(
    ...
    if node_params is None:
        raise TargetResolutionFailed("ref or selector required for select_option")
    return node_params
