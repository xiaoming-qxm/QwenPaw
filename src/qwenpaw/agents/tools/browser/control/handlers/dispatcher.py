...
def _resolved_tab_id(state: ControlState, kwargs: dict[str, Any]) -> int | None:
    try:
...
def _response_ok(response: ToolChunk) -> bool:
    try:
        return json.loads(response.content[0].text).get("ok") is True
...
async def dispatch(
    ...
    **kwargs: Any,
    ...
    if handler.meta.invalidates_snapshot and tab_id is not None and _response_ok(response):
