...
class SnapshotHandler:
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        snapshot, refs, degraded_snapshot = await build_control_snapshot(session)
        snapshot_hash = _control_snapshot_hash(snapshot)
        escalated, escalation_info = _click_effect_check(state, tab_id, snapshot_hash)
        ...
        if escalated:
            payload["escalation"] = _control_escalation_payload(escalation_info)
