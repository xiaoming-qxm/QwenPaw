    ...
    async def execute(
        ...
        if not url:
            return _json_response(
                {"ok": False, "mode": "control", "error": "url required for navigate"}
            )
        try:
        ...
        **kwargs: Any,
        ...
        try:
            await session.send_after_banner(
                "Page.navigate", {"url": url}, {"status_text": "Navigate"}
            )
        except (
            ...
            raise NavigationFailed(f"navigation to {url} failed: {exc}") from exc
        ...
        if page_settled:
            network_metadata = await _network_quiescence_wait(
                ...
                state,
                tab_id,
                timeout=control_navigation._CONTROL_NAVIGATE_NETWORK_TIMEOUT_SECONDS,
            )
