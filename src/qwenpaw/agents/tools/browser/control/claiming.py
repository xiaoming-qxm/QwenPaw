# -*- coding: utf-8 -*-
"""Tab claiming helpers for Browser Control typed handlers."""
...
from .session_manager import _control_get_existing_session, _control_get_session
...
async def _attach_tab(bridge: Any, tab_id: int, holder_id: str) -> None:
    try:
        ...
        raise CDPCommandFailed(f"Failed to attach tab {tab_id}: {exc}") from exc
...
async def claim_control_tab(
    ...
    *,
    ...
    selected_by_url = bool(url) and not _control_page_id_is_tab_id(
        raw_page_id,
    ) and index < 0
    discovered_tab_url = ""
    tab_created_by_control = False
    if selected_by_url:
        tab_id, discovered_tab_url, error, tab_created_by_control = (
        ...
        if error or tab_id is None:
            return _json_response(
                {"ok": False, "mode": "control", "error": error or "No tab selected"},
            )
    else:
    ...
    if existing is None:
        tab_id, discovered_tab_url, tab_created_by_control = await _claim_fresh(
            state,
            bridge=bridge,
            tab_id=tab_id,
            holder_id=holder_id,
    ...
    tab_url = await _control_align_tab_to_requested_url(session, url, current_url)
    ...
    return _json_response(_control_claim_success_payload(tab_id, tab_url or url))
...
async def _release_after_attach_failure(
    ...
    try:
        ...
        logger.debug("Failed to release control lease after attach failure", exc_info=True)
    except (RuntimeError, OSError, ValueError, TypeError):
        logger.debug("Failed to release control lease after attach failure", exc_info=True)
