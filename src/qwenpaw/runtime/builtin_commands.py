...
def _browser_control_prompt(
    ...
    *,
    ...
    return (
        ...
        "- Keep working toward the user's requested browser goal until it is "
        "verified complete, an explicit blocker appears, or the user stops "
        "control. A failed click, stale ref, missing structured element, or "
        "unclear page state is not a blocker by itself; re-observe and try a "
        "different browser-visible route.\n"
        ...
        "- Do not end a browser-control turn with a plan, intention, or "
        "future action. If you say you will click, type, navigate, search, "
        "open, screenshot, inspect, or try something next, the same "
        "assistant turn must include the matching browser_use tool call. "
        "Text-only answers are only for verified completion or explicit "
        "blockers.\n"
        ...
        "user asked for.\n"
        "- When the user explicitly asks for authorized reversible state "
        "changes in the current page or signed-in account, treat that as "
        "delegated for this task. Use approvals or HITL if policy requires, "
        "but do not refuse solely because it changes a signed-in account.\n"
