...
async def _load_browser_takeover(
    app: FastAPI,
    tmp_path: Path,
...
async def test_patch_plugin_disable_persists_and_unloads(
    ...
    loader = await _load_browser_takeover(app, tmp_path, monkeypatch)
    client = TestClient(app)

    response = client.patch(
        "/api/plugins/browser-takeover",
        json={"enabled": False},
    )

    assert response.status_code == 200
    assert response.json()["enabled"] is False
    record = loader.get_loaded_plugin("browser-takeover")
    assert record is not None
    assert record.enabled is False
    assert record.instance is None
    assert PluginStateStore().is_enabled("browser-takeover") is False
    listed = {
        plugin["id"]: plugin for plugin in client.get("/api/plugins").json()
    }
    assert listed["browser-takeover"]["enabled"] is False
    assert listed["browser-takeover"]["loaded"] is False

    second_app = FastAPI()
    second_loader = await _load_browser_takeover(
        second_app,
        tmp_path,
        monkeypatch,
    )
    restarted = second_loader.get_loaded_plugin("browser-takeover")
